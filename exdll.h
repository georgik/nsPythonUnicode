...
static void pushstring(char *str)
{
  ...
  mbstowcs(th->text, str, g_stringsize);
  th->next=*g_stacktop;
  *g_stacktop=th;
}
...
static char *getuservariable(int varnum)
{
  ...
  wcstombs( output_buff, g_variables+varnum*g_stringsize, wcslen(g_variables+varnum*g_stringsize)+1 );
  return output_buff;
}
...
static void setuservariable(int varnum, wchar_t *var)
{
	if (var != NULL && varnum >= 0 && varnum < __INST_LAST) {
		mbstowcs(g_variables + varnum*g_stringsize, var, g_stringsize);		
	}
}
